...
class HARFBUZZ:
    "useful definitions adapted from harfbuzz/*.h. You will need to use the constants and “macro” functions, but apart from that, see the more Pythonic wrappers defined outside this class in preference to accessing low-level structures directly."
    ...
    unicode_combining_class_func_t = ct.CFUNCTYPE(
        unicode_combining_class_t, ct.c_void_p, codepoint_t, ct.c_void_p
    )
    unicode_eastasian_width_func_t = ct.CFUNCTYPE(
        ct.c_uint, ct.c_void_p, codepoint_t, ct.c_void_p
    )
    unicode_general_category_func_t = ct.CFUNCTYPE(
        unicode_general_category_t, ct.c_void_p, codepoint_t, ct.c_void_p
    )
    unicode_mirroring_func_t = ct.CFUNCTYPE(
        codepoint_t, ct.c_void_p, codepoint_t, ct.c_void_p
    )
    unicode_script_func_t = ct.CFUNCTYPE(
        script_t, ct.c_void_p, codepoint_t, ct.c_void_p
    )
    unicode_compose_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        codepoint_t,
        codepoint_t,
        ct.POINTER(codepoint_t),
        ct.c_void_p,
    )
    unicode_decompose_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        codepoint_t,
        ct.POINTER(codepoint_t),
    ...
    unicode_decompose_compatibility_func_t = ct.CFUNCTYPE(
        ct.c_uint, ct.c_void_p, codepoint_t, ct.POINTER(codepoint_t), ct.c_void_p
    )
    ...
    SEGMENT_PROPERTIES_DEFAULT = segment_properties_t(
        DIRECTION_INVALID,
        SCRIPT_INVALID,
    ...
    buffer_message_func_t = ct.CFUNCTYPE(
        bool_t, ct.c_void_p, ct.c_void_p, ct.c_char_p, ct.c_void_p
    )
    ...
    font_get_font_extents_func_t = ct.CFUNCTYPE(
        bool_t, ct.c_void_p, ct.c_void_p, ct.POINTER(font_extents_t), ct.c_void_p
    )
    font_get_font_h_extents_func_t = font_get_font_extents_func_t
    font_get_font_v_extents_func_t = font_get_font_extents_func_t
    font_get_nominal_glyph_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        ct.c_void_p,
        codepoint_t,
    ...
    font_get_variation_glyph_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        ct.c_void_p,
        codepoint_t,
    ...
    font_get_glyph_advance_func_t = ct.CFUNCTYPE(
        position_t, ct.c_void_p, ct.c_void_p, codepoint_t, ct.c_void_p
    )
    ...
    font_get_glyph_origin_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        ct.c_void_p,
        codepoint_t,
        ct.POINTER(position_t),
        ct.POINTER(position_t),
        ct.c_void_p,
    )
    font_get_glyph_h_origin_func_t = font_get_glyph_origin_func_t
    font_get_glyph_v_origin_func_t = font_get_glyph_origin_func_t
    font_get_glyph_kerning_func_t = ct.CFUNCTYPE(
        position_t, ct.c_void_p, ct.c_void_p, codepoint_t, codepoint_t, ct.c_void_p
    )
    ...
    font_get_glyph_extents_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        ct.c_void_p,
        codepoint_t,
        ct.POINTER(glyph_extents_t),
        ct.c_void_p,
    )
    font_get_glyph_contour_point_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        ct.c_void_p,
        codepoint_t,
    ...
    font_get_glyph_name_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        ct.c_void_p,
        codepoint_t,
    ...
    font_get_glyph_from_name_func_t = ct.CFUNCTYPE(
        bool_t,
        ct.c_void_p,
        ct.c_void_p,
        ct.POINTER(ct.c_char),
    ...
    class result_class:
        ...
        def __init__(self, *args, **kwargs):
            ...
            if len(unrecognized) != 0:
                raise TypeError(
                    f"unrecognized fields for {name}: {unrecognized}"
                )
            # end if
        ...
        def to_hb(self, autoscale=None):
            ...
            for name, cttype in ctstruct._fields_:
                ...
                if val is not None:
                    if conv is not None and name in conv:
        ...
        def from_hb(celf, r, autoscale=None):
            ...
            for name, cttype in ctstruct._fields_:
                val = getattr(r, name)
                if val is not None and conv is not None and name in conv:
                    conv_from = conv[name]["from"]
                    if conv_from is HB.from_position_t:
                        if autoscale is None:
                            raise TypeError(
                                "missing autoscale arg in %s.from_hb conversion"
                                % celf.__name__
                            )
                        # end if
                        if not autoscale:
...
def _wderef(wself):
    self = wself()
    assert self is not None, "parent object has gone"
    return self
...
def def_callback_wrapper(
    celf,
    method_name,
    docstring,
    callback_field_name,
    destroy_field_name,
    def_wrap_callback_func,
    hb_proc,
):
    ...
    def set_callback(self, callback_func, user_data, destroy):
        ...
        wrap_callback_func = def_wrap_callback_func(
            wref(self), callback_func, user_data
        )
...
def def_immutable(celf, hb_query, hb_set):
    ...
    def get_immutable(self):
        return getattr(hb, hb_query)(self._hbobj) != 0
...
def seq_to_ct(seq, ct_type, conv=lambda x: x, zeroterm=False):
    "extracts the elements of a Python sequence value into a ctypes array of type ct_type, optionally applying the conv function to each value."
    nr_elts = len(seq)
...
def shaper_list_to_hb(shaper_list):
    "converts a list of strings to a null-terminated ctypes array of pointers to char. Returns a tuple of 3 items: the number of shaper strings (excluding the terminating null entry), the ctypes array, and a separate Python list of the ctypes char pointers. I’m not sure if the last one is really necessary to ensure the allocated strings don’t unexpectedly disappear, so I include it, just in case.\n\nThe shaper_list can be null, in which case 0 and null values are returned."
    if shaper_list is not None:
        ...
        c_strs = None
    # end if
    return nr_shapers, c_shaper_list, c_strs
...
hb.hb_variation_from_string.argtypes = (
    ct.c_char_p,
    ct.c_int,
    ct.POINTER(HB.variation_t),
)
hb.hb_variation_to_string.restype = None
hb.hb_variation_to_string.argtypes = (
    ct.POINTER(HB.variation_t),
    ct.c_char_p,
    ct.c_uint,
)
...
hb.hb_unicode_funcs_set_combining_class_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_funcs_set_eastasian_width_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_funcs_set_general_category_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_funcs_set_mirroring_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_funcs_set_script_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_funcs_set_compose_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_funcs_set_decompose_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_funcs_set_decompose_compatibility_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_unicode_compose.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.codepoint_t,
    ct.POINTER(HB.codepoint_t),
)
hb.hb_unicode_decompose.restype = HB.bool_t
hb.hb_unicode_decompose.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.POINTER(HB.codepoint_t),
    ct.POINTER(HB.codepoint_t),
...
hb.hb_unicode_decompose_compatibility.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.POINTER(HB.codepoint_t),
)
...
hb.hb_buffer_add_codepoints.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_int,
    ct.c_uint,
...
hb.hb_buffer_set_segment_properties.argtypes = (
    ct.c_void_p,
    ct.POINTER(HB.segment_properties_t),
)
hb.hb_buffer_get_segment_properties.restype = None
hb.hb_buffer_get_segment_properties.argtypes = (
    ct.c_void_p,
    ct.POINTER(HB.segment_properties_t),
)
...
hb.hb_buffer_set_message_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_buffer_serialize_glyphs.argtypes = (
    ct.c_void_p,
    ct.c_uint,
    ct.c_uint,
    ct.c_void_p,
...
hb.hb_buffer_deserialize_glyphs.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_int,
    ct.c_void_p,
...
hb.hb_font_get_variation_glyph.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.codepoint_t,
    ct.POINTER(HB.codepoint_t),
...
hb.hb_font_get_glyph_h_origin.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.POINTER(HB.position_t),
    ct.POINTER(HB.position_t),
)
hb.hb_font_get_glyph_v_origin.restype = HB.bool_t
hb.hb_font_get_glyph_v_origin.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.POINTER(HB.position_t),
    ct.POINTER(HB.position_t),
...
hb.hb_font_get_glyph_extents.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.POINTER(HB.glyph_extents_t),
)
hb.hb_font_get_glyph_contour_point.restype = HB.bool_t
hb.hb_font_get_glyph_contour_point.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.c_uint,
    ct.POINTER(HB.position_t),
...
hb.hb_font_get_glyph_name.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.c_char_p,
    ct.c_uint,
...
hb.hb_font_get_glyph_from_name.argtypes = (
    ct.c_void_p,
    ct.c_char_p,
    ct.c_int,
    ct.POINTER(HB.codepoint_t),
...
hb.hb_font_get_extents_for_direction.argtypes = (
    ct.c_void_p,
    HB.direction_t,
    ct.POINTER(HB.font_extents_t),
)
hb.hb_font_get_glyph_advance_for_direction.restype = None
hb.hb_font_get_glyph_advance_for_direction.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.direction_t,
    ct.POINTER(HB.position_t),
...
hb.hb_font_get_glyph_origin_for_direction.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.direction_t,
    ct.POINTER(HB.position_t),
...
hb.hb_font_add_glyph_origin_for_direction.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.direction_t,
    ct.POINTER(HB.position_t),
...
hb.hb_font_subtract_glyph_origin_for_direction.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.direction_t,
    ct.POINTER(HB.position_t),
...
hb.hb_font_get_glyph_kerning_for_direction.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.codepoint_t,
    HB.direction_t,
...
hb.hb_font_get_glyph_extents_for_origin.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    HB.direction_t,
    ct.POINTER(HB.glyph_extents_t),
...
hb.hb_font_get_glyph_contour_point_for_origin.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.c_uint,
    HB.direction_t,
...
hb.hb_font_glyph_to_string.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.c_char_p,
    ct.c_uint,
...
hb.hb_font_glyph_from_string.argtypes = (
    ct.c_void_p,
    ct.c_char_p,
    ct.c_int,
    ct.POINTER(HB.codepoint_t),
...
hb.hb_font_funcs_set_font_h_extents_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_font_v_extents_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_nominal_glyph_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_variation_glyph_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_h_advance_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_v_advance_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_h_origin_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_v_origin_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_h_kerning_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_v_kerning_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_extents_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_contour_point_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_name_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
hb.hb_font_funcs_set_glyph_from_name_func.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
if freetype is not None:  # might as well skip these if I’m not using them
...
hb.hb_ot_layout_get_glyphs_in_class.argtypes = (
    ct.c_void_p,
    HB.ot_layout_glyph_class_t,
    ct.c_void_p,
)
hb.hb_ot_layout_get_attach_points.restype = ct.c_uint
hb.hb_ot_layout_get_attach_points.argtypes = (
    ct.c_void_p,
    HB.codepoint_t,
    ct.c_uint,
    ct.POINTER(ct.c_uint),
...
hb.hb_ot_layout_get_ligature_carets.argtypes = (
    ct.c_void_p,
    HB.direction_t,
    HB.codepoint_t,
    ct.c_uint,
...
hb.hb_ot_layout_table_get_script_tags.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.POINTER(ct.c_uint),
...
hb.hb_ot_layout_table_find_script.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    HB.tag_t,
    ct.POINTER(ct.c_uint),
...
hb.hb_ot_layout_table_choose_script.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.POINTER(HB.tag_t),
    ct.POINTER(ct.c_uint),
...
hb.hb_ot_layout_table_get_feature_tags.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.POINTER(ct.c_uint),
...
hb.hb_ot_layout_script_get_language_tags.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_uint,
...
hb.hb_ot_layout_script_find_language.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    HB.tag_t,
...
hb.hb_ot_layout_language_get_required_feature_index.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_uint,
...
hb.hb_ot_layout_language_get_required_feature.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_uint,
...
hb.hb_ot_layout_language_get_feature_indexes.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_uint,
...
hb.hb_ot_layout_language_get_feature_tags.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_uint,
...
hb.hb_ot_layout_language_find_feature.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_uint,
...
hb.hb_ot_layout_feature_get_lookups.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_uint,
...
hb.hb_ot_layout_collect_lookups.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.POINTER(HB.tag_t),
    ct.POINTER(HB.tag_t),
...
hb.hb_ot_layout_lookup_collect_glyphs.argtypes = (
    ct.c_void_p,
    HB.tag_t,
    ct.c_uint,
    ct.c_void_p,
...
hb.hb_ot_layout_lookup_would_substitute.argtypes = (
    ct.c_void_p,
    ct.c_uint,
    ct.POINTER(HB.codepoint_t),
    ct.c_uint,
...
hb.hb_ot_layout_lookup_substitute_closure.argtypes = (
    ct.c_void_p,
    ct.c_uint,
    ct.c_void_p,
)
...
hb.hb_ot_layout_get_size_params.argtypes = (
    ct.c_void_p,
    ct.POINTER(ct.c_uint),
    ct.POINTER(ct.c_uint),
    ct.POINTER(ct.c_uint),
...
hb.hb_ot_tags_from_script.argtypes = (
    HB.script_t,
    ct.POINTER(HB.tag_t),
    ct.POINTER(HB.tag_t),
)
...
hb.hb_ot_shape_glyphs_closure.argtyes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_uint,
...
hb.hb_shape_plan_create.argtypes = (
    ct.c_void_p,
    ct.POINTER(HB.segment_properties_t),
    ct.c_void_p,
    ct.c_uint,
...
hb.hb_shape_plan_create_cached.argtypes = (
    ct.c_void_p,
    ct.POINTER(HB.segment_properties_t),
    ct.c_void_p,
    ct.c_uint,
...
hb.hb_shape_plan_execute.argtypes = (
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
    ct.c_void_p,
...
if hasattr(hb, "hb_ot_math_has_data"):
    ...
    hb.hb_ot_math_get_glyph_top_accent_attachment.argtypes = (
        ct.c_void_p,
        HB.codepoint_t,
    )
    ...
    hb.hb_ot_math_get_glyph_kerning.argtypes = (
        ct.c_void_p,
        HB.codepoint_t,
        HB.ot_math_kern_t,
        HB.position_t,
    ...
    hb.hb_ot_math_get_glyph_variants.argtypes = (
        ...
        ct.c_uint,
        ct.POINTER(ct.c_uint),
        ct.POINTER(HB.ot_math_glyph_variant_t),
    )
    ...
    hb.hb_ot_math_get_glyph_assembly.argtypes = (
        ct.c_void_p,
        HB.codepoint_t,
        HB.direction_t,
        ct.c_uint,
        ct.POINTER(ct.c_uint),
        ct.POINTER(HB.ot_math_glyph_part_t),
        ct.POINTER(HB.position_t),
    )
# end if
...
if hasattr(hb, "hb_ot_var_get_axis_count"):
    ...
    hb.hb_ot_var_get_axes.argtypes = (
        ct.c_void_p,
        ct.c_uint,
        ct.POINTER(ct.c_uint),
        ct.POINTER(HB.ot_var_axis_t),
    )
    hb.hb_ot_var_find_axis.restype = HB.bool_t
    hb.hb_ot_var_find_axis.argtypes = (
        ct.c_void_p,
        HB.tag_t,
        ct.POINTER(ct.c_uint),
        ct.POINTER(HB.ot_var_axis_t),
    ...
    hb.hb_ot_var_normalize_variations.argtypes = (
        ct.c_void_p,
        ct.POINTER(HB.variation_t),
        ct.c_uint,
        ct.POINTER(ct.c_int),
    ...
    hb.hb_ot_var_normalize_coords.argtypes = (
        ct.c_void_p,
        ct.c_uint,
...
def version_atleast(major, minor, micro):
    ...
    return hb.hb_version_atleast(major, minor, micro) != 0
...
def version_string():
    ...
    return hb.hb_version_string().decode()  # automatically stops at NUL?
...
def direction_from_string(s):
    sb = s.encode()
    return hb.hb_direction_from_string(sb, len(sb))
...
def direction_to_string(direction):
    return hb.hb_direction_to_string(direction).decode()  # automatically stops at NUL?
...
class Language:
    "wraps the hb_language_t opaque type. Do not instantiate directly; use the from_string() and default() methods."
    ...
    def default(celf):
        ...
        return Language(hb.hb_language_get_default())
    ...
    def to_string(self):
        ...
        return hb.hb_language_to_string(
            self._hbobj
        ).decode()  # automatically stops at NUL?

    # end to_string

    def __repr__(self):
        return "<Language>(%s)" % self.to_string()
...
def tag_from_bytes(b):
    ...
    return hb.hb_tag_from_string(b, len(b))
...
def tag_from_string(s):
    ...
    sb = s.encode()
    return hb.hb_tag_from_string(sb, len(sb))
...
def script_from_iso15924_tag(tag):
    return hb.hb_script_from_iso15924_tag(tag)
...
def script_from_string(s):
    sb = s.encode()
    return hb.hb_script_from_string(sb, len(sb))
...
def script_to_iso15924_tag(script):
    return hb.hb_script_to_iso15924_tag(script)
...
def script_get_horizontal_direction(script):
    return hb.hb_script_get_horizontal_direction(script)
...
class UnicodeFuncs:
    "wrapper around hb_unicode_funcs_t objects. Do not instantiate directly; use the create, get_default and get_empty methods."
    ...
    def __del__(self):
        if hb is not None and self._hbobj is not None:
            hb.hb_unicode_funcs_destroy(self._hbobj)
            self._hbobj = None
        # end if
    ...
    def get_default():
        return UnicodeFuncs(
            hb.hb_unicode_funcs_reference(hb.hb_unicode_funcs_get_default())
        )
    ...
    def create(parent=None):
        if parent is not None and not isinstance(parent, UnicodeFuncs):
            raise TypeError("parent must be None or a UnicodeFuncs")
        # end if
        return UnicodeFuncs(
            hb.hb_unicode_funcs_create(
                (lambda: None, lambda: parent._hbobj)[parent is not None]()
            )
        )
    ...
    def parent(self):
        return UnicodeFuncs(
            hb.hb_unicode_funcs_reference(hb.hb_unicode_funcs_get_parent(self._hbobj))
        )
    ...
    def combining_class(self, unicode):
        ...
        return hb.hb_unicode_combining_class(self._hbobj, unicode)

    # end combining_class

    def eastasian_width(self, unicode):
        "invokes the eastasian_width_func."
        return hb.hb_unicode_eastasian_width(self._hbobj, unicode)

    # end eastasian_width

    def general_category(self, unicode):
        "invokes the general_category_func."
        return hb.hb_unicode_general_category(self._hbobj, unicode)

    # end general_category

    def mirroring(self, unicode):
        "invokes the mirroring_func."
        return hb.hb_unicode_mirroring(self._hbobj, unicode)

    # end mirroring

    def script(self, unicode):
        "invokes the script_func."
        return hb.hb_unicode_script(self._hbobj, unicode)
    ...
    def decompose_compatibility(self, u):
        ...
        decomposed_len = hb.hb_unicode_decompose_compatibility(
            self._hbobj, u, decomposed
        )
        return decomposed[:decomposed_len]
...
def def_unicodefuncs_extra():
    def def_wrap_combining_class_func(wself, combining_class_func, user_data):
        ...
        def wrap_combining_class_func(c_funcs, unicode, c_user_data):
            return combining_class_func(_wderef(wself), unicode, user_data)

        # end wrap_combining_class_func

        # begin def_wrap_combining_class_func
        return wrap_combining_class_func
    ...
    def def_wrap_eastasian_width_func(wself, eastasian_width_func, user_data):
        ...
        def wrap_eastasian_width_func(c_funcs, unicode, c_user_data):
            return eastasian_width_func(_wderef(wself), unicode, user_data)

        # end wrap_eastasian_width_func

        # begin def_wrap_eastasian_width_func
        return wrap_eastasian_width_func
    ...
    def def_wrap_general_category_func(wself, general_category_func, user_data):
        ...
        def wrap_general_category_func(c_funcs, unicode, c_user_data):
            return general_category_func(_wderef(wself), unicode, user_data)

        # end wrap_general_category_func

        # begin def_wrap_general_category_func
        return wrap_general_category_func
    ...
    def def_wrap_mirroring_func(wself, mirroring_func, user_data):
        ...
        def wrap_mirroring_func(c_funcs, unicode, c_user_data):
            return mirroring_func(_wderef(wself), unicode, user_data)

        # end wrap_mirroring_func

        # begin def_wrap_mirroring_func
        return wrap_mirroring_func
    ...
    def def_wrap_script_func(wself, script_func, user_data):
        ...
        def wrap_script_func(c_funcs, unicode, c_user_data):
            return script_func(_wderef(wself), unicode, user_data)

        # end wrap_script_func

        # begin def_wrap_script_func
        return wrap_script_func
    ...
    def def_wrap_decompose_func(wself, decompose_func, user_data):
        ...
        return wrap_decompose_func

    # end def_wrap_decompose_func

    def def_wrap_decompose_compatibility_func(
        wself, decompose_compatibility_func, user_data
    ):
        ...
        return wrap_decompose_compatibility_func
    ...
    for basename, def_func, protostr, resultstr in (
        (
            "combining_class",
            def_wrap_combining_class_func,
            "combining_class_func(self, unicode, user_data)",
            "n integer combining class code",
        ),
        (
            "eastasian_width",
            def_wrap_eastasian_width_func,
            "eastasian_width_func(self, unicode, user_data)",
            "n integer",
        ),
        (
            "general_category",
            def_wrap_general_category_func,
            "general_category_func(self, unicode, user_data)",
            "n integer general category code",
        ),
        (
            "mirroring",
            def_wrap_mirroring_func,
            "mirroring_func(self, unicode, user_data)",
            " unicode code point",
        ),
        (
            "script",
            def_wrap_script_func,
            "script_func(self, unicode, user_data)",
            "n integer script code",
        ),
        (
            "compose",
            def_wrap_compose_func,
            "compose_func(self, a, b, user_data)",
            " unicode code point or None",
        ),
        (
            "decompose",
            def_wrap_decompose_func,
            "decompose_func(self, ab, user_data)",
            " 2-tuple of Unicode code points or None",
        ),
        (
            "decompose_compatibility",
            def_wrap_decompose_compatibility_func,
            "decompose_compatibility_func(self, u, user_data)",
            " tuple of Unicode code points or None",
        ),
    ):
        def_callback_wrapper(
            celf=UnicodeFuncs,
            method_name="set_%s_func" % basename,
            docstring="sets the %(name)s_func callback, along with an optional destroy"
            " callback for the user_data. The callback_func should be declared"
            " as follows:\n"
            "\n"
            "    def %(proto)s\n"
...
class Blob:
    "wraps the hb_blob_t opaque type. This is used to manage storage for Face objects. Do not instantiate directly; use the create and get_empty methods."
    ...
    def create(celf, data, length, mode, user_data, destroy):
        "low-level Blob create routine: data is address of storage and length is its length in bytes."
        if destroy is not None:
    ...
    def create_for_array(celf, arr, mode):
        "creates a Blob that wraps the storage for the specified bytes, bytearray or array.array object. mode is one of the MEMORY_MODE_XXX values."
    ...
    def create_sub(self, offset, length):
        "creates a sub-Blob spanning the specified range of bytes in the parent. Child inherits parent’s memory mode, but Parent is made immutable."
        return Blob(hb.hb_blob_create_sub_blob(self._hbobj, offset, length))

    # end create_sub

    @staticmethod
    def get_empty():
        "returns the (unique) empty Blob. This has a null data pointer, zero length, and its memory mode is readonly."
        return Blob(hb.hb_blob_reference(hb.hb_blob_get_empty()))

    # end get_empty

    def __len__(self):
        "the length of the Blob data."
        return hb.hb_blob_get_length(self._hbobj)

    # end __len__

    def data(self, writable):
        "returns a tuple (addr, length) of the Blob for read-only or read/write access, depending on writable."
        length = ct.c_uint()
...
GlyphInfo = def_struct_class(name="GlyphInfo", ctname="glyph_info_t")


class GlyphPositionExtra:
    # extra members for GlyphPosition class.

    if qahirah is not None:

        @property
        def advance(self):
            "returns the x- and y-advance of the glyph position as a qahirah.Vector."
            return qahirah.Vector(self.x_advance, self.y_advance)
        ...
        def offset(self):
            ...
            return qahirah.Vector(self.x_offset, self.y_offset)
...
class SegmentPropertiesExtra:
    ...
    def __hash__(self):
        return hb.hb_segment_properties_hash(self._hbobj)
...
class Buffer:
    "a HarfBuzz buffer. This is where the text shaping is actually done. Note that a Buffer can only manage a single run of text (language, script, direction = “segment”) at once. Do not instantiate directly; call the create or get_empty methods."
    ...
    def create():
        ...
        return Buffer(hb.hb_buffer_create())
    ...
    def get_empty():
        ...
        return Buffer(hb.hb_buffer_reference(hb.hb_buffer_get_empty()))
    ...
    def clear_contents(self):
        "similar to reset, but does not clear the Unicode functions or the replacement code point."
    ...
    def allocation_successful(self):
        return hb.hb_buffer_allocation_successful(self._hbobj) != 0
    ...
    def add_codepoints(self, text, text_length, item_offset, item_length):
        ...
        hb.hb_buffer_add_codepoints(
            self._hbobj, c_text, text_length, item_offset, item_length
        )
        self.check_alloc()
    ...
    def content_type(self):
        return hb.hb_buffer_get_content_type(self._hbobj)
    ...
    def direction(self):
        ...
        return hb.hb_buffer_get_direction(self._hbobj)
    ...
    def script(self):
        ...
        return hb.hb_buffer_get_script(self._hbobj)
    ...
    def flags(self):
        ...
        return hb.hb_buffer_get_flags(self._hbobj)
    ...
    def cluster_level(self):
        ...
        return hb.hb_buffer_get_cluster_level(self._hbobj)
    ...
    def __len__(self):
        ...
        return hb.hb_buffer_get_length(self._hbobj)
    ...
    def segment_properties(self):
        ...
        return SegmentProperties.from_hb(result)
    ...
    def unicode_funcs(self):
        return UnicodeFuncs(
            hb.hb_unicode_funcs_reference(hb.hb_buffer_get_unicode_funcs(self._hbobj))
        )
    ...
    def glyph_infos(self):
        ...
        return tuple(GlyphInfo.from_hb(arr[i]) for i in range(nr_glyphs.value))
    ...
    def glyph_positions(self):
        ...
        return tuple(
            GlyphPosition.from_hb(arr[i], self.autoscale)
            for i in range(nr_glyphs.value)
        )
    ...
    if qahirah is not None:
        ...
        def get_glyphs(self, origin=None):
            "returns a a tuple of two items, a list of qahirah.Glyph objects, and the Vector origin for the following glyph run. All positions can be optionally offset by the specified Vector origin. The signs of the y-coordinates are flipped to correspond to the usual Cairo convention of increasing downwards."
            ...
            for i in range(len(glyph_infos)):
                result.append(
                    qahirah.Glyph(
                        glyph_infos[i].codepoint, pos + flip * glyph_positions[i].offset
                    )
                )
    ...
    def replacement_codepoint(self):
        return hb.hb_buffer_get_replacement_codepoint(self._hbobj)
    ...
    def serialize_glyphs(self, font, format, flags):
        ...
        result = b""
        while pos != end:
            seg_items = hb.hb_buffer_serialize_glyphs(
                self._hbobj,
                pos,
                end,
                tempbuf.buffer_info()[0],
                tempbuf.buffer_info()[1],
                ct.byref(buf_consumed),
                (lambda: None, lambda: font._hbobj)[font is not None](),
                format._tag,
                flags,
            )
...
class SerializeFormat:
    "a high-level encapsulation of hb_buffer_serialize_format_t values. Do not instantiate directly; use from_string and get_list methods."
    ...
    def from_string(celf, s):
        sb = s.encode()
        return celf(hb.hb_buffer_serialize_format_from_string(sb, len(sb)))

    # end from_string

    def to_string(self):
        return hb.hb_buffer_serialize_format_to_string(self._tag).decode()
    ...
    def __repr__(self):
        return f"{type(self).__name__}({HB.UNTAG(self._tag, True)})"
...
class Face:
    "wrapper around hb_face_t objects, analogous to Cairo’s font_face_t objects. Do not instantiate directly; use create or get_empty methods."
    ...
    def __new__(celf, _hbobj, autoscale):
        ...
        if self is None:
            self = super().__new__(celf)
            assert autoscale is not None, "missing autoscale flag"
            ...
            assert autoscale is None or autoscale == self.autoscale, (
                "inconsistent autoscale settings"
            )
    ...
    def create_for_tables(reference_table, user_data, destroy, autoscale):
        "creates a Face which calls the specified reference_table action to access the font tables. This should be declared as\n\n    def reference_table(face, tag, user_data)\n\nwhere face is the Face instance, tag is the integer tag identifying the table, and must return either None or a Blob, which should remain valid for as long as the Face is referenced. The interpretation of user_data is up to you. The destroy action may be None, but if not, it should be declared as\n\n     def destroy(user_data)\n\nand will be called when the Face is destroyed."

        @HB.reference_table_func_t
        def wrap_reference_table(c_face, c_tag, c_user_data):
            result = reference_table(
                Face(hb.hb_face_reference(c_face), autoscale), c_tag, user_data
            )
        ...
        result = Face(
            hb.hb_face_create_for_tables(wrap_reference_table, None, wrap_destroy),
            autoscale,
        )
    ...
    def get_empty():
        ...
        return Face(hb.hb_face_reference(hb.hb_face_get_empty()), False)
    ...
    if freetype is not None:
        ...
        def ft_create(ft_face):
            ...
            # end if
            return Face(hb.hb_ft_face_create_referenced(ft_face._ftobj), True)
    ...
    def reference_table(self, tag):
        ...
        return Blob(hb.hb_face_reference_table(self._hbobj, tag))

    # end reference_table

    def reference_blob(self):
        "returns the Blob with which the Face was created, or the empty Blob if it doesn’t have one."
        return Blob(hb.hb_face_reference_blob(self._hbobj))
    ...
    def ot_layout_has_glyph_classes(self):
        return hb.hb_ot_layout_has_glyph_classes(self._hbobj) != 0

    # end ot_layout_has_glyph_classes

    def ot_layout_get_glyph_class(self, glyph):
        return hb.hb_ot_layout_get_glyph_class(self._hbobj, glyph)
    ...
    def ot_layout_get_attach_points(self, glyph):
        ...
        # end while
        return tuple(point_array[i] for i in range(nr_attach_points))
    ...
    def ot_layout_table_get_script_tags(self, table_tag):
        ...
        # end while
        return tuple(script_tags[i] for i in range(nr_script_tags))
    ...
    def ot_layout_find_script(self, table_tag, script_tag):
        ...
        if (
            hb.hb_ot_layout_table_find_script(
                self._hbobj, table_tag, script_tag, script_index
            )
            != 0
        ):
    ...
    def ot_layout_choose_script(self, table_tag, script_tags):
        ...
        success = (
            hb.hb_ot_layout_table_choose_script(
                self._hbobj, table_tag, c_script_tags, script_index, chosen_script
            )
            != 0
        )
        return (success, script_index.value, chosen_script.value)
    ...
    def ot_layout_table_get_feature_tags(self, table_tag):
        ...
        # end while
        return tuple(feature_tags[i] for i in range(nr_feature_tags))
    ...
    def ot_layout_script_get_language_tags(self, table_tag, script_index):
        ...
        while True:
            ...
            if language_tags is not None:
                break
            # allocate space, now I know how much I need
            language_count = ct.c_uint(nr_language_tags)
            language_tags = (nr_language_tags * ct.c_uint)()
        # end while
        return tuple(language_tags[i] for i in range(nr_language_tags))
    ...
    def ot_layout_script_find_language(self, table_tag, script_index, language_tag):
        ...
        success = (
            hb.hb_ot_layout_script_find_language(
                self._hbobj, table_tag, script_index, language_tag, language_index
            )
            != 0
        )
        return (success, language_index.value)
    ...
    def ot_layout_language_get_required_feature(
        self, table_tag, script_index, language_index
    ):
        feature_index = ct.c_uint()
        feature_tag = HB.tag_t()
        success = (
            hb.hb_ot_layout_language_get_required_feature(
                self._hbobj,
                table_tag,
                script_index,
                language_index,
        ...
        return (success, feature_index.value, feature_tag.value)
    ...
    def ot_layout_language_get_feature_indexes(
        ...
        # end while
        return tuple(feature_indexes[i] for i in range(nr_feature_indexes))
    ...
    def ot_layout_language_find_feature(
        self, table_tag, script_index, language_index, feature_tag
    ):
        feature_index = ct.c_uint()
        success = (
            hb.hb_ot_layout_language_find_feature(
                self._hbobj,
                table_tag,
                script_index,
                language_index,
        ...
        return (success, feature_index.value)
    ...
    def ot_layout_feature_get_lookups(self, table_tag, feature_index):
        ...
        # end while
        return tuple(lookup_indexes[i] for i in range(nr_lookups))

    # end ot_layout_feature_get_lookups

    def ot_layout_table_get_lookup_count(self, table_tag):
        return hb.hb_ot_layout_table_get_lookup_count(self._hbobj, table_tag)
    ...
    def ot_layout_collect_lookups(self, table_tag, scripts, languages, features):
        ...
        if features is not None:
            seq_to_ct(features, HB.tag_t, zeroterm=True)
        else:
            pass
        # end if
        lookup_indexes = Set.to_hb()
        hb.hb_ot_layout_collect_lookups(
            self._hbobj,
            table_tag,
            c_scripts,
            c_languages,
            cC_features,
            lookup_indexes._hbobj,
        )
        return lookup_indexes.from_hb()
    ...
    def ot_layout_lookup_collect_glyphs(
        ...
        hb.hb_ot_layout_lookup_collect_glyphs(
            self._hbobj,
            table_tag,
            lookup_index,
            glyphs_before._hbobj,
            glyphs_input._hbobj,
            glyphs_after._hbobj,
            glyphs_output._hbobj,
        )
        return (
            glyphs_before.from_hb(),
            glyphs_input.from_hb(),
            glyphs_after.from_hb(),
            glyphs_output.from_hb(),
        )
    ...
    def ot_layout_has_substitution(self):
        return hb.hb_ot_layout_has_substitution(self._hbobj) != 0
    ...
    def ot_layout_lookup_substitute_closure(self, lookup_index):
        glyphs = Set.to_hb()
        hb.hb_ot_layout_lookup_substitute_closure(
            self._hbobj, lookup_index.glyphs._hbobj
        )
        return glyphs.from_hb()
    ...
    def ot_layout_has_positioning(self):
        return hb.hb_ot_layout_has_positioning(self._hbobj) != 0
    ...
    if hasattr(hb, "hb_ot_math_has_data"):
        ...
        def ot_math_has_data(self):
            return hb.hb_ot_math_has_data(self._hbobj) != 0

        # end ot_math_has_data

        def ot_math_is_glyph_extended_shape(self, glyph):
            return hb.hb_ot_math_is_glyph_extended_shape(self._hbobj, glyph) != 0
    ...
    if hasattr(hb, "hb_ot_var_get_axis_count"):
        ...
        def ot_var_nr_axes(self):
            return hb.hb_ot_var_get_axis_count(self._hbobj)
        ...
        def ot_var_axes(self):
            ...
            return list(OTVarAxis.from_hb(a) for a in axes_array)
        ...
        def ot_var_find_axis(self, axis_tag):
            ...
            if found:
                ...
                axis_info = None
            # end if
            return c_axis_index.value, axis_info

        # end ot_var_find_axis

        def ot_var_normalize_variations(self, variations):
            if not isinstance(variations, (tuple, list)) or not (
                isinstance(v, Variation) for v in variations
            ):
            ...
            hb.hb_ot_var_normalize_variations(
                self._hbobj, c_variations, nr_variations, coords, nr_coords
            )
            return list(coords)
        ...
        def ot_var_normalize_coords(self, design_coords):
            ...
            hb.hb_ot_var_normalize_coords(
                self._hbobj, nr_design_coords, c_design_coords, c_normalized_coords
            )
            return list(c_normalized_coords)
...
def def_face_props(celf):
    ...
    def def_face_prop(propname):
        ...
        def getter(self):
            return getattr(hb, hb_getter)(self._hbobj)
...
GlyphPart = def_struct_class(
    ...
    conv={
        "start_connector_length": {"from": HB.from_position_t, "to": HB.to_position_t},
        "end_connector_length": {"from": HB.from_position_t, "to": HB.to_position_t},
        "full_advance": {"from": HB.from_position_t, "to": HB.to_position_t},
    },
)
...
class Font:
    "wrapper around hb_font_t objects, analogous to Cairo’s scaled_font_t objects. Do not instantiate directly; use create methods."
    ...
    def get_variation_glyph(self, unicode, variation_selector):
        glyph = HB.codepoint_t()
        if (
            hb.hb_font_get_variation_glyph(
                self._hbobj, unicode, variation_selector, ct.byref(glyph)
            )
            != 0
        ):
            result = glyph.value
    ...
    def get_glyph_h_origin(self, glyph):
        ...
        y = HB.position_t()
        if (
            hb.hb_font_get_glyph_h_origin(self._hbobj, glyph, ct.byref(x), ct.byref(y))
            != 0
        ):
            if self.autoscale:
    ...
    def get_glyph_v_origin(self, glyph):
        ...
        y = HB.position_t()
        if (
            hb.hb_font_get_glyph_v_origin(self._hbobj, glyph, ct.byref(x), ct.byref(y))
            != 0
        ):
            if self.autoscale:
                result = (HB.from_position_t(x.value), HB.from_position_t(y.value))
    ...
    def get_glyph_contour_point(self, glyph, point_index):
        ...
        y = HB.position_t()
        if (
            hb.hb_font_get_glyph_contour_point(
                self._hbobj, glyph, point_index, ct.byref(x), ct.byref(y)
            )
            != 0
        ):
            if self.autoscale:
    ...
    def get_glyph_from_name(self, name):
        ...
        if (
            hb.hb_font_get_glyph_from_name(
                self._hbobj, c_name, len(name), ct.byref(c_glyph)
            )
            != 0
        ):
    ...
    def get_extents_for_direction(self, glyph, direction):
        ...
        hb.hb_font_get_extents_for_direction(
            self._hbobj, glyph, direction, ct.byref(extents)
        )
        return FontExtents.from_hb(extents, self.autoscale)
    ...
    def get_glyph_advance_for_direction(self, glyph, direction):
        x = HB.position_t()
        y = HB.position_t()
        hb.hb_font_get_glyph_advance_for_direction(
            self._hbobj, glyph, direction, ct.byref(x), ct.byref(y)
        )
        if self.autoscale:
            result = (HB.from_position_t(x.value), HB.from_position_t(y.value))
        else:
    ...
    def get_glyph_origin_for_direction(self, glyph, direction):
        x = HB.position_t()
        y = HB.position_t()
        hb.hb_font_get_glyph_origin_for_direction(
            self._hbobj, glyph, direction, ct.byref(x), ct.byref(y)
        )
        if self.autoscale:
            result = (HB.from_position_t(x.value), HB.from_position_t(y.value))
    ...
    def add_glyph_origin_for_direction(self, glyph, direction, origin):
        ...
        # end if
        hb.hb_font_add_glyph_origin_for_direction(
            self._hbobj, glyph, direction, ct.byref(x), ct.byref(y)
        )
        if self.autoscale:
            result = (HB.from_position_t(x.value), HB.from_position_t(y.value))
    ...
    def subtract_glyph_origin_for_direction(self, glyph, direction, origin):
        ...
        if self.autoscale:
            ...
            y = HB.position_t(origin[1])
        # end if
        hb.hb_font_subtract_glyph_origin_for_direction(
            self._hbobj, glyph, direction, ct.byref(x), ct.byref(y)
        )
        if self.autoscale:
            result = (HB.from_position_t(x.value), HB.from_position_t(y.value))
    ...
    def get_glyph_kerning_for_direction(self, first_glyph, second_glyph, direction):
        x = HB.position_t()
        y = HB.position_t()
        hb.hb_font_get_glyph_kerning_for_direction(
            self._hbobj, first_glyph, second_glyph, direction, ct.byref(x), ct.byref(y)
        )
        if self.autoscale:
            result = (HB.from_position_t(x.value), HB.from_position_t(y.value))
        else:
    ...
    def get_glyph_extents_for_direction(self, glyph, direction):
        ...
        if (
            hb.hb_font_get_glyph_extents_for_origin(
                self._hbobj, glyph, direction, ct.byref(extents)
            )
            != 0
        ):
    ...
    def get_glyph_contour_point_for_direction(self, glyph, point_index, direction):
        ...
        y = HB.position_t()
        if (
            hb.hb_font_get_glyph_contour_point_for_origin(
                self._hbobj, glyph, point_index, direction, ct.byref(x), ct.byref(y)
            )
            != 0
        ):
            if self.autoscale:
    ...
    def glyph_to_string(self, glyph):
        ...
        return c_name.value.decode()  # nul-terminated
    ...
    def set_funcs(self, funcs, font_data, destroy):
        if funcs is not None and not isinstance(funcs, FontFuncs):
        ...
        hb.hb_font_set_funcs(
            self._hbobj,
            (lambda: None, lambda: funcs._hbobj)[funcs is not None](),
            self._hbobj,
            wrap_destroy,
        )
    ...
    @property
    def parent(self):
        return Font(hb.hb_font_reference(hb.hb_font_get_parent(self._hbobj)), None)

    # end parent

    @parent.setter
    def parent(self, new_parent):
        if new_parent is not None and not isinstance(new_parent, Font):
            raise TypeError("new_parent must be None or a Font")
        # end if
        hb.hb_font_set_parent(
            self._hbobj, (lambda: None, lambda: new_parent._hbobj)[new_parent is not None]()
        )
    ...
    def face(self):
        return Face(
            hb.hb_face_reference(hb.hb_font_get_face(self._hbobj)), self.autoscale
        )
    ...
    if freetype is not None:
        ...
        def ft_load_flags(self):
            return hb.hb_ft_font_get_load_flags(self._hbobj)
        ...
        @property
        def ft_face(self):
            return freetype.Face(None, hb.hb_ft_font_get_face(self._hbobj), None)
    ...
    def ot_layout_get_ligature_carets(self, direction, glyph):
        ...
        while True:
            ...
            if caret_array is not None:
                break
            # allocate space, now I know how much I need
            caret_count = ct.c_uint(nr_carets)
            caret_array = (nr_carets * HB.position_t)()
        # end while
        return tuple(caret_array[i] for i in range(nr_carets))
    ...
    if hasattr(hb, "hb_ot_math_has_data"):
        ...
        def ot_math_get_glyph_italics_correction(self, glyph):
            return hb.hb_ot_math_get_glyph_italics_correction(self._hbobj, glyph)
        ...
        def ot_math_get_glyph_kerning(self, glyph, kern, correction_height):
            ...
            result = hb.hb_ot_math_get_glyph_kerning(
                self._hbobj, glyph, kern, correction_height
            )
            if self.autoscale:
                result = HB.from_position_t(result)
            # end if
        ...
        def ot_math_get_nr_glyph_variants(self, glyph, direction):
            ...
            return hb.hb_ot_math_get_glyph_variants.argtypes(
                self._hbobj,
                glyph,
                direction,
                0,  # start_offset
        ...
        def ot_math_get_glyph_variants(self, glyph, direction):
            ...
            return list(GlyphVariant.from_hb(v, self.autoscale) for v in c_variants)
        ...
        def ot_math_get_nr_glyph_assembly_parts(self, glyph, direction):
            "direction is a DIRECTION_XXX value."
            return hb.hb_ot_math_get_glyph_assembly(
                self._hbobj,
                glyph,
                direction,
                0,  # start_offset
        ...
        def ot_math_get_glyph_assembly(self, glyph, direction):
            ...
            # end if
            return parts, italics_correction
...
def def_font_extra(celf):
    def def_prop(propname, proptype, docextra, conv_to_hb, conv_from_hb):
        ...
        assert (conv_to_hb is None) == (conv_from_hb is None)

        if conv_to_hb is not None:
        ...
        getter.__name__ = propname
        getter.__doc__ = "the {name} property as an (x, y) tuple.{extra}".format(
            name=propname,
            extra=(lambda: "", lambda: " " + docextra)[docextra is not None](),
        )
        setter.__name__ = propname
...
class FontFuncs:
    "wrapper around hb_font_funcs_t objects. Do not instantiate directly; use create and get_empty methods. The autoscale attribute indicates whether your callbacks return floating-point metrics that need to be multiplied by 64 to convert to HarfBuzz position_t values."
    ...
    def __new__(celf, _hbobj, autoscale):
        ...
        if self is None:
            ...
            assert autoscale is None or self.autoscale == autoscale, (
                "inconsistent autoscale settings"
            )
...
def def_fontfuncs_extra():
    def get_font(c_font):
        return Font(hb.hb_font_reference(c_font), None)
    ...
    def def_wrap_get_variation_glyph_func(wself, get_variation_glyph, user_data):
        ...
        def wrap_get_variation_glyph(
            c_font, c_font_data, unicode, variation_selector, c_glyph, c_user_data
        ):
            font = get_font(c_font)
            glyph = get_variation_glyph(
                font, font._font_data, unicode, variation_selector, user_data
            )
            if glyph is not None:
                c_glyph[0] = glyph
            # end if
        ...
        return wrap_get_variation_glyph
    ...
    def def_wrap_get_glyph_kerning_func(wself, get_glyph_kerning, user_data):
        ...
        def wrap_get_glyph_kerning(
            c_font, c_font_data, first_glyph, second_glyph, c_user_data
        ):
            font = get_font(c_font)
            result = get_glyph_kerning(
                font, font._font_data, first_glyph, second_glyph, user_data
            )
            if _wderef(wself).autoscale:
                result = HB.to_position_t(result)
            else:
    ...
    def def_wrap_get_glyph_extents_func(wself, get_glyph_extents, user_data):
        ...
        def wrap_get_glyph_extents(c_font, c_font_data, glyph, c_extents, c_user_data):
            ...
            # end if
            return extents is not None

        # end wrap_get_glyph_extents

        # begin def_wrap_get_glyph_extents_func
        return wrap_get_glyph_extents

    # end def_wrap_get_glyph_extents_func

    def def_wrap_get_glyph_contour_point_func(
        wself, get_glyph_contour_point, user_data
    ):
        @HB.font_get_glyph_contour_point_func_t
        def wrap_get_glyph_contour_point(
            c_font, c_font_data, glyph, point_index, c_x, c_y, c_user_data
        ):
            font = get_font(c_font)
            pos = get_glyph_contour_point(
                font, font._font_data, glyph, point_index, user_data
            )
            if pos is not None:
                if _wderef(wself).autoscale:
                    c_x[0] = HB.to_position_t(pos[0])
        ...
        return wrap_get_glyph_contour_point
    ...
    def def_wrap_get_glyph_name_func(wself, get_glyph_name, user_data):
        ...
        def wrap_get_glyph_name_func(
            c_font, c_font_data, glyph, c_name, size, c_user_data
        ):
            font = get_font(c_font)
        ...
        return wrap_get_glyph_name_func
    ...
    def def_wrap_get_glyph_from_name_func(wself, get_glyph_from_name, user_data):
        ...
        def wrap_get_glyph_from_name(
            c_font, c_font_data, c_name, c_len, c_glyph, c_user_data
        ):
            if c_len >= 0:
        ...
        return wrap_get_glyph_from_name
    ...
    for basename, def_func, protostr, resultstr in (
        (
            "font_h_extents",
            def_wrap_get_font_extents_func,
            "get_font_h_extents(font, font_data, user_data)",
            " FontExtents or None",
        ),
        (
            "font_v_extents",
            def_wrap_get_font_extents_func,
            "get_font_v_extents(font, font_data, user_data)",
            " FontExtents or None",
        ),
        (
            "nominal_glyph",
            def_wrap_get_nominal_glyph_func,
            "get_nominal_glyph(font, font_data, unicode, user_data)",
            "n integer glyph code or None",
        ),
        (
            "variation_glyph",
            def_wrap_get_variation_glyph_func,
            "get_variation_glyph(font, font_data, unicode, variation_selector, user_data)",
            "n integer glyph code or None",
        ),
        (
            "glyph_h_advance",
            def_wrap_get_glyph_advance_func,
            "get_glyph_h_advance(font, font_data, glyph, user_data)",
            " float",
        ),
        (
            "glyph_v_advance",
            def_wrap_get_glyph_advance_func,
            "get_glyph_v_advance(font, font_data, glyph, user_data)",
            " float",
        ),
        (
            "glyph_h_origin",
            def_wrap_get_glyph_origin_func,
            "get_glyph_h_origin(font, font_data, glyph, user_data)",
            " 2-tuple, qahirah.Vector or None",
        ),
        (
            "glyph_v_origin",
            def_wrap_get_glyph_origin_func,
            "get_glyph_v_origin(font, font_data, glyph, user_data)",
            " 2-tuple, qahirah.Vector or None",
        ),
        (
            "glyph_h_kerning",
            def_wrap_get_glyph_kerning_func,
            "get_glyph_h_kerning(font, font_data, first_glyph, second_glyph, user_data)",
            " float",
        ),
        (
            "glyph_v_kerning",
            def_wrap_get_glyph_kerning_func,
            "get_glyph_v_kerning(font, font_data, first_glyph, second_glyph, user_data)",
            " float",
        ),
        (
            "glyph_extents",
            def_wrap_get_glyph_extents_func,
            "get_glyph_extents(font, font_data, glyph, user_data)",
            " GlyphExtents or None",
        ),
        (
            "glyph_contour_point",
            def_wrap_get_glyph_contour_point_func,
            "get_glyph_contour_point(font, font_data, glyph, point_index, user_data)",
            " 2-tuple, qahirah.Vector or None",
        ),
        (
            "glyph_name",
            def_wrap_get_glyph_name_func,
            "get_glyph_name_func(font, font_data, glyph, user_data)",
            " string or None",
        ),
        (
            "glyph_from_name",
            def_wrap_get_glyph_from_name_func,
            "get_glyph_from_name(font, font_data, name, user_data)",
            "n integer glyph code or None",
        ),
    ):
        def_callback_wrapper(
...
class FeatureExtra:
    ...
    def to_string(self):
        ...
        return namebuf.value.decode()  # automatically stops at NUL
...
def shape_full(font, buffer, features=None, shaper_list=None):
    ...
    result = (
        hb.hb_shape_full(
            font._hbobj, buffer._hbobj, c_features, nr_features, c_shaper_list
        )
        != 0
    )
    buffer.autoscale = font.autoscale
...
class Set:
    "wrapper around hb_set_t objects. For internal use only: all relevant functions will pass and return Python sets."
    ...
    def from_hb(self):
        if self._hbobj is not None:
            result = set()
            first = HB.codepoint_t()
            last = HB.codepoint_t()
            first.value = HB.SET_VALUE_INVALID
            while (
                hb.hb_set_next_range(self._hbobj, ct.byref(first), ct.byref(last)) != 0
            ):
...
def ot_tags_from_script(script):
    ...
    return (script_tag_1.value, script_tag_2.value)
...
def ot_tag_to_script(tag):
    return hb.hb_ot_tag_to_script(tag).value
...
def ot_tag_from_language(language):
    if not isinstance(language, Language):
        raise TypeError("language must be a Language")
    # end if
    return hb.hb_ot_tag_from_language(language._hbobj)
...
def ot_tag_to_language(tag):
    return Language(hb.hb_ot_tag_to_language(tag))
...
def ot_shape_glyphs_closure(font, buffer, features, glyphs=None):
    ...
    if features is not None:
        ...
        nr_features = 0
    # end if
    hb.hb_ot_shape_glyphs_closure(
        font._hbobj, buffer._hbobj, c_features, nr_features, c_glyphs._hbobj
    )
    buffer.autoscale = font.autoscale
    return c_glyphs.from_hb()
...
class ShapePlan:
    "wrapper around hb_shape_plan_t objects. Do not instantiate directly; use create and get_empty methods."
    ...
    def create(face, props, user_features, shaper_list, cached):
        ...
        if user_features is not None:
            c_user_features = seq_to_ct(
                user_features, HB.feature_t, lambda f: f.to_hb()
            )
    ...
    def execute(self, font, buffer, features):
        ...
        result = (
            hb.hb_shape_plan_execute(
                self._hbobj, font._hbobj, buffer._hbobj, c_features, nr_features
            )
            != 0
        )
        buffer.autoscale = font.autoscale
    ...
    @staticmethod
    def get_empty():
        return ShapePlan(hb.hb_shape_plan_reference(hb.hb_shape_plan_get_empty()))
